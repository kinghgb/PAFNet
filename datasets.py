...
def make_dataset(root):
    if isinstance(root,str):
        ...
        imgs = [(os.path.join(image_path, img_name), os.path.join(mask_path, os.path.splitext(img_name)[0] + '.png'),os.path.join(edge_path, os.path.splitext(img_name)[0] + '.png')) for img_name in img_list]
        return imgs
    else:
        imgs = []
        for root_dir in root:
            ...
            img_list = [f for f in os.listdir(image_path) if f.endswith('.jpg') or f.endswith('png')]
            sub_imgs = [
                (os.path.join(image_path, img_name), os.path.join(mask_path, os.path.splitext(img_name)[0] + '.png'),os.path.join(edge_path, os.path.splitext(img_name)[0] + '.png'))
...
#     def __init__(self, root, img_transform=None, gt_transform=None):
#         self.root = root
